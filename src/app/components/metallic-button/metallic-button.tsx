...
interface MetallicButtonProps {
  children: ReactNode; // Accepts any valid React node (text, icons, etc.)
  onClick?: () => void; // Optional click handler
}

const MetallicButton: React.FC<MetallicButtonProps> = ({ children, onClick }) => {
  return (
    <div className="relative rounded-3xl flex justify-center items-center flex-row gap-3">
      <button
        className="flex group relative h-10 w-36 p-[1.5px] rounded-lg border border-neutral-400 bg-gradient-to-b from-neutral-100/50 from-30% via-neutral-600 to-pink-200 shadow-lg shadow-pink-100 active:shadow-md active:shadow-pink-100 active:scale-[99%] active:translate-y-[0.5px] overflow-hidden"
        onClick={onClick}
      >
